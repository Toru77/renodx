...
cbuffer _Globals : register(b0)
{
  ...
  float CubeMapIntensity : packoffset(c60.y) = {1};
  float CubeMapFresnel : packoffset(c60.z) = {0};
  float2 DuDvMapImageSize : packoffset(c61) = {256,256};
  float2 DuDvScale : packoffset(c61.z) = {4,4};
  float BloomIntensity : packoffset(c62) = {1};
  float ReflectionIntensity : packoffset(c62.y) = {0};
  float ReflectionFresnel : packoffset(c62.z) = {0};
  ...
  out float4 o7 : TEXCOORD10)
{
  ...
  p3.xy = v2.xy * UVaDuDvTexcoord.zw + UVaDuDvTexcoord.xy;
  r0.x = dot(v4.xyz, World._m00_m10_m20);
  r0.y = dot(v4.xyz, World._m01_m11_m21);
  r0.z = dot(v4.xyz, World._m02_m12_m22);
