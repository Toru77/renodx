...
) : SV_Target
{
    ...
    uint source0SamplerIdx = (uint)source0SamplerIdxClamped + 0u;
    float4 source0Sample = srv2DTextures[source0TexIndex].Sample(g_bindlessSamplerState[source0SamplerIdx], float2(TEXCOORD.x, TEXCOORD.y));

    float3 source0RGB = min(source0Sample.rgb, 65504.0f);

    uint source4TexIndex = ResolveBindlessIndex(source4Tex2dIndex, 17);
    int  source4SamplerIdxClamped = (int)min((uint)(g_defaultSamplerStateIndices[1].y), (uint)255);
    uint source4SamplerIdx = (uint)source4SamplerIdxClamped + 0u;
    float4 source4Sample = srv2DTextures[source4TexIndex].Sample(g_bindlessSamplerState[source4SamplerIdx], float2(TEXCOORD.x, TEXCOORD.y));

    float3 source4Premult = source4Sample.rgb * source4Sample.w;

    float source4InvAlpha0 = 1.0f - source4Sample.w;
    float source4InvAlpha  = saturate(source4InvAlpha0);

    float3 source0Weighted = source4InvAlpha * source0RGB;
    float3 blended         = source4Premult + source0Weighted;

    uint source1TexIndex = ResolveBindlessIndex(source1Tex2dIndex, 17);
    int  source1SamplerIdxClamped = (int)min((uint)(g_defaultSamplerStateIndices[0].z), (uint)255);
    uint source1SamplerIdx = (uint)source1SamplerIdxClamped + 0u;
    float4 source1Sample = srv2DTextures[source1TexIndex].Sample(g_bindlessSamplerState[source1SamplerIdx], float2(TEXCOORD.x, TEXCOORD.y));

    float3 sum = blended + source1Sample.rgb;
    ...
    float4 lutSample = srv3DTextures[lutTexIndex].SampleLevel(g_staticSampler_TrilinearClamp, lutCoord, 0.0f);
    ...
    float radialDistSq = dot(float2(TEXCOORD.z, TEXCOORD.w), float2(TEXCOORD.z, TEXCOORD.w));
