...
) : SV_Target
{
    ...
    uint source2TexIndex = ResolveBindlessIndex(source2Tex2dIndex, 17);
    int  source2SamplerIdxClamped = (int)min((uint)(g_defaultSamplerStateIndices[0].z), (uint)255);
    uint source2SamplerIdx = (uint)source2SamplerIdxClamped + 0u;
    float4 source2Sample = srv2DTextures[source2TexIndex].Sample(g_bindlessSamplerState[source2SamplerIdx], float2(TEXCOORD.x, TEXCOORD.y));

    float3 source2Modulated = source2Sample.rgb * source1Sample.rgb * ps_pixel_size.z;

    float3 sum = source0RGB + source1Sample.rgb + source2Modulated;
