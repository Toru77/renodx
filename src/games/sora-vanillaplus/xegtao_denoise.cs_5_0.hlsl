...
void main(uint2 dispatch_thread_id : SV_DispatchThreadID)
{
  ...
  if (pix_coord_base.x >= width || pix_coord_base.y >= height) return;
  GTAOConstants consts = BuildGTAOConstants(uint2(width, height));
  const bool is_last_pass = xegtao_denoise_is_last_pass >= 0.5;
  XeGTAO_Denoise(pix_coord_base, consts, g_srcWorkingAOTerm, g_srcWorkingEdges, g_samplerPointClamp, g_outFinalAOTerm, is_last_pass);
}