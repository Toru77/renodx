...
GTAOConstants BuildGTAOConstants(uint2 viewport_size)
{
  ...
  if (depth_linearize_mul * depth_linearize_add < 0.0) depth_linearize_add = -depth_linearize_add;
...
void GetQualityParameters(out lpfloat slice_count, out lpfloat steps_per_slice)
{
  ...
  if (quality_level == 0u) { slice_count = 3.0; steps_per_slice = 3.0; }
  else if (quality_level == 1u) { slice_count = 6.0; steps_per_slice = 3.0; }
  else { slice_count = 9.0; steps_per_slice = 3.0; }
}
...
bool TryMapWorkingPixelToMrtTexel(uint2 working_pix_coord, uint2 working_size, out uint2 mapped_mrt_texel)
{
  ...
  if (mrt_width == 0u || mrt_height == 0u) return false;
...
float3 SafeNormalize3(float3 value, float3 fallback_value)
{
  ...
  if (len2 < 1e-5) return fallback_value;
...
float3 BuildDepthFallbackNormal(uint2 pix_coord, GTAOConstants consts)
{
  ...
  lpfloat4 edges_lrtb = XeGTAO_CalculateEdges(viewspace_z, pix_lz, pix_rz, pix_tz, pix_bz);
  ...
  float4 edges_f = edges_lrtb;
  float3 depth_normal = XeGTAO_CalculateNormal(edges_f, center, left, right, top, bottom);
...
float3 BuildSelectedInputNormal(uint2 pix_coord, uint2 working_size, GTAOConstants consts)
{
  ...
  if (xegtao_mrt_normal_available >= 0.5) {
    ...
    if (!TryMapWorkingPixelToMrtTexel(pix_coord, working_size, mapped_mrt_texel)) return selected;
    ...
    if (decoded_len2 >= 1e-5) {
      ...
      if (xegtao_normal_darkening_mode < 0.5) final_blend *= saturate(xegtao_normal_max_darkening);
...
void main(uint2 pix_coord : SV_DispatchThreadID)
{
  ...
  if (pix_coord.x >= width || pix_coord.y >= height) return;
  
  GTAOConstants consts = BuildGTAOConstants(uint2(width, height));
  lpfloat slice_count, steps_per_slice;
  ...
  XeGTAO_MainPass(pix_coord, slice_count, steps_per_slice, SpatioTemporalNoise(pix_coord, noise_index), selected_normal, consts, g_srcWorkingDepth, g_samplerPointClamp, g_outWorkingAOTerm, g_outWorkingEdges);
}