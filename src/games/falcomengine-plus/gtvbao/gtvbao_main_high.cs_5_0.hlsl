///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// falcomengine-plus GTVBAO — Pass 2: Main GTAO (High quality)
//
// Kai-style: builds GTAOConstants in-shader, gets quality from push_constants.
// Quality is determined by GTVBAO_quality push constant, not hardcoded slices.
...
void main(uint2 p : SV_DispatchThreadID)
{
  ...
  if (g_isfast_enabled > 0.5f) {
    if (GTVBAO_noise_type < 0.5f)      n = SpatioTemporalNoise_ISFAST(p, noise_idx);
    else if (GTVBAO_noise_type < 1.5f) n = SpatioTemporalNoise_IGN(p, noise_idx);
