  ...
  out uint4 o1 : SV_Target1)
{
  ...
  if (!ssgi_enabled && shader_injection_data.xegtao_ssgi_bound < 0.5) {
    // Zero out only when neither Falcom SSGI nor XeGTAO SSGI is active.
    r15.xyzw = float4(0, 0, 0, 0);
  } else if (shader_injection_data.xegtao_ssgi_bound < 0.5) {
    // Apply Falcom SSGI tuning only to vanilla SSGI, not XeGTAO GI.
