...
cbuffer _Globals : register(b0)
{
  ...
  float Shininess : packoffset(c60) = {0.5};
  float SpecularPower : packoffset(c60.y) = {50};
  float3 SpecularColor : packoffset(c61) = {1,1,1};
  ...
  float ShadowReceiveOffset : packoffset(c63.z) = {0.600000024};
  float SphereMapIntensity : packoffset(c63.w) = {1};
  float BloomIntensity : packoffset(c64) = {0.699999988};
  float4 GameEdgeParameters : packoffset(c65) = {1,1,1,0.00300000003};
  float4 OutlineColorFactor : packoffset(c66) = {1,1,1,1};
  float3 OutlineColor : packoffset(c67) = {0.699999988,0.5,0.300000012};
  ...
  out float4 o7 : TEXCOORD5)  // DLAA: prevClip
{
  ...
  o0.xyzw = r1.xyzw;
  o6.xyzw = r1.xyzw;
  o1.xyz = OutlineColor.xyz * OutlineColorFactor.xyz;
  o1.w = 1;
