...
cbuffer _Globals : register(b0)
{
  ...
  float PerMaterialMainLightClampFactor : packoffset(c52.y) = {1.5};
  float3 LightDirForChar : packoffset(c53);
  float GameMaterialID : packoffset(c53.w) = {0};
  ...
  float3 SpecularColor : packoffset(c64.y) = {1,1,1};
  float3 RimLitColor : packoffset(c65) = {1,1,1};
  float RimLitIntensity : packoffset(c65.w) = {4};
  float RimLitPower : packoffset(c66) = {2};
  float RimLightClampFactor : packoffset(c66.y) = {2};
  float ShadowReceiveOffset : packoffset(c66.z) = {0.600000024};
  float BloomIntensity : packoffset(c66.w) = {0.699999988};
  float GlareIntensity : packoffset(c67) = {1};
  float4 GameEdgeParameters : packoffset(c68) = {1,1,1,0.00300000003};
  float4 PointLightParams : packoffset(c69) = {0,0,0,0};
  float4 PointLightColor : packoffset(c70) = {0,0,0,0};
}
