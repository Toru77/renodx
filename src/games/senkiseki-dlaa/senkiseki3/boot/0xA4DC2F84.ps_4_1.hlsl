// ---- Created with 3Dmigoto v1.4.1 on Thu Aug  6 04:18:16 2026
...
cbuffer _Globals : register(b0)
{
  ...
  float Shininess : packoffset(c72) = {0.5};
  float SpecularPower : packoffset(c72.y) = {50};
  float3 SpecularColor : packoffset(c73) = {1,1,1};
  float3 RimLitColor : packoffset(c74) = {1,1,1};
  float RimLitIntensity : packoffset(c74.w) = {4};
  float RimLitPower : packoffset(c75) = {2};
  float RimLightClampFactor : packoffset(c75.y) = {2};
  float ShadowReceiveOffset : packoffset(c75.z) = {0.600000024};
  float SphereMapIntensity : packoffset(c75.w) = {1};
  float BloomIntensity : packoffset(c76) = {0.699999988};
  float4 GameEdgeParameters : packoffset(c77) = {1,1,1,0.00300000003};
  float4 OutlineColorFactor : packoffset(c78) = {1,1,1,1};
  float3 OutlineColor : packoffset(c79) = {0.699999988,0.5,0.300000012};
...
void main(
  ...
  float4 v9 : TEXCOORD5,  // DLAA: prevClip
  uint v8 : SV_SampleIndex0,
  out float4 o0 : SV_TARGET0,
  out float4 o1 : SV_TARGET1,
  out float4 o2 : SV_TARGET2,
  out float4 o3 : SV_TARGET3)  // DLAA: appended 16-bit motion RTV
{
  ...
  r4.z = r2.w + r2.w;
  r4.xz = r3.xy * -r4.zz + -r4.xy;
  r4.xz = r4.xz * float2(0.5,0.5) + float2(0.5,0.5);
  r4.y = 1 + -abs(r4.y);
  r4.y = r4.y * r4.y;
  r2.w = r2.w * r2.w;
  r2.w = r4.y * r2.w;
  r4.xyz = SphereMapSampler.Sample(LinearClampSamplerState_s, r4.xz).xyz;
  r4.w = dot(LightDirForChar.xyz, r3.xyz);
  r5.xyz = SpecularColor.xyz * Light0.m_colorIntensity.xyz;
  r0.xyw = r0.xyw * r1.www + LightDirForChar.xyz;
  r1.w = dot(r0.xyw, r0.xyw);
  r1.w = rsqrt(r1.w);
  ...
  r6.x = r4.w * 0.5 + 0.5;
  r6.y = 0;
  r0.y = CartoonMapSampler.SampleLevel(LinearClampSamplerState_s, r6.xy, 0).x;
  r6.xyz = Light0.m_colorIntensity.xyz * r0.yyy + scene.GlobalAmbientColor.xyz;
  r6.xyz = min(PerMaterialMainLightClampFactor, r6.xyz);
  r0.xyw = r5.xyz * r0.xxx + r6.xyz;
  r0.z = 1 + -r0.z;
  r5.xyz = r0.xyw * scene.MiscParameters1.xyz + -r0.xyw;
  r0.xyz = r0.zzz * r5.xyz + r0.xyw;
  r0.w = 1 + -abs(r3.w);
  r0.w = max(0, r0.w);
  r0.w = log2(r0.w);
  ...
  r5.xyz = min(float3(1,1,1), r0.xyz);
  r5.xyz = float3(1,1,1) + -r5.xyz;
  r5.xyz = ShadowColorShift.xyz * r5.xyz;
  r0.xyz = r5.xyz * r2.xyz + r0.xyz;
  r2.xyz = SphereMapIntensity * r4.xyz;
  ...
  if (DLAA_PER_OBJECT_MOTION > 0.5f) {
    float2 prevNDC = v9.xy / max(v9.w, 0.001);
