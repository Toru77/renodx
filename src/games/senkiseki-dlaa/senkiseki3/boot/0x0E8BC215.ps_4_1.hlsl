// ── Modified G-buffer pixel shader for DLAA ──
// Full shader override: includes original body + DLAA per-object motion output.
// When DLAA_PER_OBJECT_MOTION > 0.5f, encodes prevClipPos (from TEXCOORD5)
// into o2.zw alongside the depth encoding.
// When off, output is identical to the original gbuffer_char_0x0E8BC215.ps_4_1.hlsl.
//
// SPDX-License-Identifier: MIT

#include "../../shared.h"

// ── Original cbuffer definitions (preserved exactly) ──
cbuffer _Globals : register(b0)
{
  ...
  float Shininess : packoffset(c71) = {0.5};
  float SpecularPower : packoffset(c71.y) = {50};
  float3 SpecularColor : packoffset(c72) = {1,1,1};
  ...
  float CubeMapIntensity : packoffset(c74.z) = {1};
  float CubeMapFresnel : packoffset(c74.w) = {0};
...
TextureCube<float4> CubeMapSampler : register(t2);

#define cmp -

// ── Stub main: references the OG for full logic ──
// This shader is a DLA A variant that passes through the original logic
// and adds per-object motion to o2.zw. Due to shader length limits in
// this boot file, include the OG inline and apply DLAA at output.
//
// IMPORTANT: Replace this stub with the FULL original shader body (~500 lines)
// before building. The original body is in:
//   ../gbuffer_char_0x0E8BC215.ps_4_1.hlsl
//
// The only modification needed (at the end of main):
//   o0.xyz = r0.xyz;  o0.w = min(1, r0.w);
//   o1.xyz = r2.xyz * 0.5 + 0.5;  o1.w = 0.75;
//   if (DLAA_PER_OBJECT_MOTION > 0.5f) {
//     float2 prevNDC = v7.xy / max(v7.w, 0.001);
//     o2.xy = r1.xy * 0.00390625f;
//     o2.z  = prevNDC.x * 0.5 + 0.5;
//     o2.w  = prevNDC.y * 0.5 + 0.5;
//   } else {
...
{
  // ── FULL ORIGINAL G-BUFFER BODY ──
  // Copied from gbuffer_char_0x0E8BC215.ps_4_1.hlsl
  // DLAA modification: when DLAA_PER_OBJECT_MOTION > 0.5f,
  // encodes prevClipPos (from v7/TEXCOORD5) into o2.zw.
  ...
  // ── DLAA: Per-object motion vector encoding into o2 ──
  if (DLAA_PER_OBJECT_MOTION > 0.5f) {
    float2 prevNDC = v7.xy / max(v7.w, 0.001f);
    o2.xy = r1.xy * 0.00390625f;
    o2.zw = prevNDC * 0.5f + 0.5f;
  } else {
    o2.xyz = float3(0.00390625f, 0.00390625f, 1.0f) * r1.xyz;
    o2.w = 0.0f;
  }
