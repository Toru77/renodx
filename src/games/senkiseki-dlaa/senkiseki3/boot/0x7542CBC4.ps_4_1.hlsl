...
cbuffer _Globals : register(b0)
{
  ...
  float ShadowReceiveOffset : packoffset(c72) = {0.600000024};
...
Texture2D<float4> CartoonMapSampler : register(t2);
...
void main(
  ...
  float4 v8 : TEXCOORD5,  // DLAA: prevClip
  ...
  out float4 o3 : SV_TARGET3)  // DLAA: appended 16-bit motion RTV
{
  ...
  r3.x = r1.w * 0.5 + 0.5;
  r3.y = 0;
  r1.w = CartoonMapSampler.SampleLevel(LinearClampSamplerState_s, r3.xy, 0).x;
  ...
  if (DLAA_PER_OBJECT_MOTION > 0.5f) {
    float2 prevNDC = v8.xy / max(v8.w, 0.001);
