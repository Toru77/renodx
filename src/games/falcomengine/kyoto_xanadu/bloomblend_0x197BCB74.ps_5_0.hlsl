...
// ---- Created with 3Dmigoto v1.4.1 on Sat Jul 18 23:52:56 2026

cbuffer cb_local : register(b3)
{
  float4 uvClamp0_g : packoffset(c0);
  float4 uvClamp1_g : packoffset(c1);
  float2 uvClamp2_g : packoffset(c2);
  float lum_g : packoffset(c2.z);
  float intensity_g : packoffset(c2.w);
  float force_glow_alpha_g : packoffset(c3);
  float force_glow_near_g : packoffset(c3.y);
  float force_glow_range_inv_g : packoffset(c3.z);
}
...
float4 blendBloomSrgb(SamplerState samLinear_s, float4 v1) {
  ...
  r0.xyzw = min(uvClamp0_g.xyzw, v1.xyxy);
  ...
  r2.xyzw = min(uvClamp1_g.xyzw, v1.xyxy);
    ...
    out float4 o0 : SV_Target0)
{
    ...
    float4 clampedUV0 = min(uvClamp0_g, v1.xyxy);
    ...
    float4 clampedUV1 = min(uvClamp1_g, v1.xyxy);
    ...
    float2 clampedUV2 = min(uvClamp2_g.xy, v1.xy);
